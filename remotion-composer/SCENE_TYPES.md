...
| `brand_card` | `BrandCardScene` | *(all optional)* | `brandName`, `tagline`, `ctaText`, `productImage`, `hardwareTreatment`, `accentColor`, `backgroundColor`, `sceneDurationSeconds` | Animated end card: letters spring up individually, accent underline draws in, tagline fades, CTA appears in accent color, wordmark pulses once at 4 s. Text-only by default; product visuals require an approved `productImage` or explicit synthetic hardware treatment. |
