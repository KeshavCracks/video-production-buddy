...
| `brand_card` | `BrandCardScene` | *(all optional)* | `brandName`, `tagline`, `ctaText`, `accentColor`, `backgroundColor`, `sceneDurationSeconds` | Animated end card: letters spring up individually, accent underline draws in, tagline fades, CTA appears in accent color, wordmark pulses once at 4 s. Use for CTA/brand beats. |
