...
Use `genui_interaction` by default for G3 when the narration is long enough
that section-by-section review is clearer than a terminal block. It should
decide whether linear chat is sufficient; for multi-section narration review,
route to GenUI and delegate to `genui_session`. Generate a project-specific
`ui_session_config` with the full script sections, word-count status,
required-line confirmations, and approve/revise actions.
After submission, read and validate `ui_session_response`, summarize requested changes
