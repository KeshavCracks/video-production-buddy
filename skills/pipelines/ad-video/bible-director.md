...
Use the full retrieved card payload, not only `summary`:
- Adapt `principles` into concise `script_usage.usage_note` language.
- Adapt `execution_techniques` into `scene_usage.visual_or_pacing_instruction`
  when the card has visual, pacing, audio, format, color, sound, music, or editing targets.
- Copy or adapt applicable `avoid_when` and `failure_patterns` into
  `do_not_overapply` so downstream stages can avoid common misuses.
- Copy `cross_domain_notes` when present so co-present craft domains can be
  checked by `check_ad_video_planning_knowledge_alignment()`.
...
Use `genui_interaction` by default for G-I when the local browser path is
available. It should decide that linear chat is not sufficient because the round
requires multi-axis selection across CONFIRMED, CHALLENGED, and FLAGGED
dimension verdicts, then delegate to `genui_session`. Generate a project-specific
`ui_session_config` that shows verdicts, lets the user keep brief choices or accept
research-backed recommendations, and records CTA/execution concerns for Step
7b. After submission, read and validate `ui_session_response`, summarize choices, and
only then update `production_bible`. The GenUI path must not write canonical
artifacts directly: it must not write `production_bible`, `decision_log`, or
checkpoints.
...
| confidence=default-heuristic, verdict=INSUFFICIENT-DATA | **Silent** — resolve in favour of enriched_brief; do NOT surface |

`confidence=default-heuristic` entries must use `verdict=INSUFFICIENT-DATA`.
Do not emit `SUPPORTED` or `CONTRADICTED` with default-heuristic confidence;
the intelligence schema rejects that shape because a heuristic guess cannot
override or confirm the approved enriched brief.
