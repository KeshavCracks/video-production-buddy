...
drafting scenes. For every selected entry whose `application_targets` include
`scene_plan`, `visual`, `pacing`, `format`, `color`, `sound`, `music`, or
`editing`, or whose
...
- `knowledge_alignment_notes`: a concrete visual, pacing, format, color, sound,
  music, or editing instruction showing how the professional producer knowledge
  shapes the scene
...
## GenUI v7 Scene Plan Approval Gate

The full scene plan is a visual, timing, product-visibility, and risk review.
Before setting `user_approved: true`, use `genui_interaction` by default and
serve a GenUI v7 gate workspace.

Build an `interaction_request` that produces a `ui_session_config` with:

- `ScenePlanReview` for every planned scene, duration, transition, crop region,
  product visibility, product reference requirement, and hallucination check.
- `RevisionPatch` fields bound to `scene_plan.human_feedback.*` for timing,
  product-visibility, crop, and motion-coherence changes.
- `ApprovalChecklist` requiring explicit approval of total timing, scene order,
  product visibility, and hallucination-risk controls.
- `ArtifactTracePanel` linking back to `script`, `production_bible`,
  `idea_options`, and the mode supplement used for the plan.

Wait for `ui_session_response`. Validate and summarize the response before
writing `scene_plan.user_approved: true` or any revision request. CLI fallback is
allowed only when `genui_session execution fails` or the user explicitly
declines the browser path. A returned localhost URL counts as browser path
available. In fallback, mirror the same ScenePlanReview, RevisionPatch,
