...
### Step 2a: GenUI Workspace-First Path

For the Creative Requirements Worksheet, use `genui_interaction` by default
when the local browser path is available. It should decide that linear chat is
not sufficient for G-0 because the round has many dimensions, defaults, and
structured revision fields, then delegate to `genui_session`. Generate a
project-specific `ui_session_config` for Gate G-0 with all required worksheet
dimensions, prefilled values, recommended defaults, help text, and bindings to
the future `enriched_brief.creative_requirements.*.value` fields.

After the user submits the form, read and validate the `ui_session_response`. Summarize
...
after validating `ui_session_response` and applying the normal review/checkpoint
protocol.
