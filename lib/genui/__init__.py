...
def get_browser_url(url: str) -> str:
    ...
    On WSL2, 127.0.0.1 is accessible from Windows via localhost forwarding,
    but presenting 'localhost' is more natural for the user.
    """
    if is_wsl2():
...
def cleanup_orphaned_servers(project_dir: Path | str) -> int:
    """Find and kill all orphaned GenUI server processes for a project.

    Returns the number of processes cleaned up.
    """
    project_root = Path(project_dir)
    ui_dir = project_root / "artifacts" / FORM_DIRNAME
...
def wait_for_response(
    ...
    *,
    ...
    """Poll for a GenUI response file to appear.

    Returns the parsed response dict, or None if the timeout expires.
    """
    import time
...
def check_availability() -> dict[str, Any]:
    """Return a summary of GenUI availability for agent decision-making.

    The agent should call this before deciding whether to use GenUI or CLI.
    """
    from tools.tool_registry import registry

    registry.discover()
    tool = registry.get("genui_form")
    available = tool is not None and tool.get_info().get("status") == "available"
    return {
        "available": available,
        "wsl2": is_wsl2(),
        "browser_url_note": (
