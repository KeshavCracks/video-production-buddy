...
The audit is intentionally permissive: we accept either an explicit URL
(http/https) OR a named publication / report / campaign whose name carries
its own credibility. We only flag the case where the rationale says nothing
specific at all — "the data clearly shows X" without naming the data.
"""
...
# These are specific brand / firm names — false positives from substring
# matching are negligible (unlikely to embed "Nielsen" inside another word).
...
def _has_citable_evidence(text: str) -> bool:
    ...
    substring matching (their brand-name uniqueness makes false positives
    negligible). Generic-noun signals require word-boundary matching so
    'reportedly', 'studying', 'campaigning' do NOT pass — they describe
    activity, not citation."""
    if not text or not text.strip():
        return False
    if _URL_PATTERN.search(text):
        return True
    lowered = text.lower()
    if any(token in lowered for token in _NAMED_ENTITY_TOKENS):
        return True
    return _GENERIC_TOKEN_RE.search(text) is not None


def audit_intelligence_provenance(intelligence_brief: dict[str, Any]) -> list[dict[str, str]]:
