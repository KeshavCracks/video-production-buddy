...
class WanxImage(BaseTool):
    ...
    idempotency_key_fields = ["prompt", "model", "operation", "size", "seed", "n"]
        ...
        if operation in ("image_editing", "image_to_image_set"):
            if not model_meta.get("supports_editing"):
                return ToolResult(
                    success=False,
                    error=(
                        f"Operation '{operation}' requires wanx2.1-t2i-turbo "
                        f"or wanx2.1-t2i-plus. Current model: '{model}'."
                    ),
                )
    ...
    def _attach_ref_images(
        ...
        for item in ref_images:
            ...
            resolved.append(b64 if b64 else item)
