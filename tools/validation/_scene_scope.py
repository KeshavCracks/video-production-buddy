...
def validate_scene_id_list(value: Any, field_name: str) -> tuple[list[str], list[str], bool]:
    ...
    normalized = [
        scene_id.strip() if isinstance(scene_id, str) else scene_id
        for scene_id in value
    ]
    invalid_indexes = [
        idx
        for idx, scene_id in enumerate(normalized)
    ...
    duplicate_ids = sorted(
        scene_id for scene_id, count in Counter(normalized).items() if count > 1
    )
    issues = [
        f"{field_name} contains duplicate scene ids: {duplicate_ids}."
    ] if duplicate_ids else []
    return normalized, issues, True
