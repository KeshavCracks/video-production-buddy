...
| Video Production Buddy path                               | Upstream path                             |
| ---------------------------------------------- | ----------------------------------------- |
...
The `gsap` upstream skill is NOT re-vendored — Video Production Buddy already ships its
own GSAP skill family under `.agents/skills/gsap*/`.

## Local edits

Any divergence from upstream is noted at the top of the edited file as an
HTML comment starting with `Video Production Buddy-local`. Current edits:
...
- `hyperframes-cli/SKILL.md` — added `validate` to the command list and a
  ...
  Video Production Buddy's HyperFrames runtime path relies on `hyperframes validate` as
...
1. Video Production Buddy contributors may not have the HyperFrames monorepo on disk.
2. Skills must be readable from the Video Production Buddy tree for agent discovery.
